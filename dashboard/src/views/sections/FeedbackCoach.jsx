...
export function FeedbackCoach({ vectors, peerMatrix, forceUserEmail }) {
  ...
  // Filter Peer Matrix if forcing a specific user
  ...
  return (
    <section className="space-y-6">
          ...
          {coaching.top_strengths && coaching.top_strengths.length > 0 ? (
            <ul className="coach-list">
              {coaching.top_strengths.map((str, i) => (
                <li key={i}>
                  <span className="coach-dot success" />
                  <span>{str}</span>
                </li>
              ))}
            </ul>
          ) : (
            <div style={{ color: 'var(--color-text-muted)', fontStyle: 'italic' }}>No strengths data available</div>
          )}
        </GlassCard>
          ...
          {coaching.top_improvements && coaching.top_improvements.length > 0 ? (
            <ul className="coach-list">
              {coaching.top_improvements.map((imp, i) => (
                <li key={i}>
                  <span className="coach-dot warning" />
                  <span>{imp}</span>
                </li>
              ))}
            </ul>
          ) : (
            <div style={{ color: 'var(--color-text-muted)', fontStyle: 'italic' }}>No improvement data available</div>
          )}
        </GlassCard>
      </div>
      ...
      <div className="grid-2col">
        ...
        {growth.trend && growth.trend !== 'insufficient_data' && (
          <GlassCard style={{ padding: 'var(--space-6)' }}>
            ...
            <div className="chart-holder" style={{ height: 192 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[
                  { period: 'First Half', score: growth.first_half_avg },
                  { period: 'Second Half', score: growth.second_half_avg },
                ]} margin={{ top: 10, right: 10, bottom: 0, left: -20 }}>
                  <XAxis dataKey="period" stroke="var(--chart-grid)" tick={{ fill: 'var(--color-text-primary)', fontSize: 13 }} />
        ...
        {visiblePeerMatrix && visiblePeerMatrix.length > 0 && (
          <GlassCard style={{ padding: 'var(--space-6)' }}>
            <div className="section-subheader">Peer Review Matrix</div>
                ...
                <tbody>
                  {visiblePeerMatrix.map((peer, i) => (
                    <tr key={i}>
                      ...
                      <td>{peer.reviewee}</td>
                      <td>
                        <span className="badge badge-info">{peer.focus_area}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div style={{ marginTop: 'var(--space-6)' }}>
        <FeedbackThread 
          targetEmail={selectedV.email} 
          targetName={selectedV.name}
        />
      </div>
    </section>
  );
